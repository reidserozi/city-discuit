...
func New(db *sql.DB, conf *config.Config) (*Server, error) {
	...
	r.Handle("/api/digest_unsubscribe", s.withHandler(s.digestUnsubscribe)).Methods("POST", "GET")
...
func getIPBlockedResponse(siteName string) []byte {
	return []byte(`
<!DOCTYPE html>
<html lang="en">
  <head>
...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if match, err := s.ipblocks.Match(httputil.GetIP(r)); err != nil {
		...
		w.Write(getIPBlockedResponse(s.config.SiteName))
		return
	}
